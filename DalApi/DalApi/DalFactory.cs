...
namespace DalApi
{
    public class DalFactory
    {
        public static IDal GetDal()
        {
            ...
            if (type == null) throw new DalConfigException($"Class {dalNamespace}.{dalClass} was not found in the {dalPkg}.dll");

            IDal dal = (IDal)type.GetProperty("Instance",
                      BindingFlags.Public | BindingFlags.Static).GetValue(null);
            if (dal == null) throw new DalConfigException($"Class {dalNamespace}.{dalClass} is not a singleton or wrong propertry name for Instance");
